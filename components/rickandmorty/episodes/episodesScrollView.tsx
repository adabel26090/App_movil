import {
    ActivityIndicator, Alert, FlatList, SafeAreaView, StyleSheet, Text,
...
export function EpisodesScrollView() {
    ...
    const [data, setData] = useState<EpisodesResult>({  // Datos de los personajes y la información adicional
        info: {
            pages: 0,
    ...
    return (
        <SafeAreaView style={styles.content}>  {/* Contenedor principal */}
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <EpisodesCard episodes={item} />  // Renderiza una tarjeta para cada personaje
                )}
